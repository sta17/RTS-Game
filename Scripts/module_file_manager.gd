...
static func File_Exists(path:String) -> bool:
	...
	printerr("MODULE_FILE_MANAGER: File does not exist! path: ",path)
	return false

static func Encode_Data_File(path:String) -> void:
	if !File_Exists(path):return
	
	var load_file_data:String = FileAccess.open(path,FileAccess.READ).get_as_text()
	var save_file_path: String = path.replace(M_C.FORMAT_CSVDATA,M_C.FORMAT_DATA)
